...
def evaluate_interpolation(interp_type, t):
    ...
    chordwise (Kaplan) or spherical-angle (Deriaz) coordinate t in [0, 1],
    returns a monotonically increasing weight in [0, 1] used to blend the
    local blade angle beta between its inlet value (beta1, at t=0) and its
    outlet value (beta2, at t=1):
        beta(t) = beta1 + (beta2 - beta1) * evaluate_interpolation(t)
    ...
    chord:
      - Cubic (Standard):  3t^2 - 2t^3.   Smooth S-curve, zero slope at
        both ends (Hermite smoothstep). Peak loading around 30-40% chord.
      ...
      - Inlet Loaded:      1-(1-t)^2.     Steeper deflection near the
        inlet; maximum loading at the leading edge.
      - Outlet Loaded:     t^2.           Steeper deflection near the
        outlet; maximum loading at the trailing edge.

    All five satisfy the two boundary conditions f(0)=0, f(1)=1 exactly,
    so beta always equals beta1 at the leading edge and beta2 at the
    trailing edge regardless of which law is selected; the choice of law
    only changes how beta transitions between those two fixed endpoints."""
...
def compute_kaplan_backbone(RPM, Q0, Hn, g, eta_h, eta_v, eta_o, sigma_target,
                             ...
                             R_hub, R_tip, L_z):
    """Axial (Kaplan) hydrodynamic model: derives and integrates the blade
    mean-surface backbone directly from the turbomachinery Euler equation,
    assuming an infinite number of zero-thickness blades (the classic
    textbook idealization that perfectly guides the flow with no slip and
    no blockage).
    ...
    After the backbone is built, the reference chord length is measured at
    the mean hydraulic radius R_m = sqrt((R_tip^2 + R_hub^2) / 2), and the
    optimal blade count Z is derived from a target chord-to-pitch ratio
    (solidity), clamped to a sensible [3, 12] range.

    Returns a dict with the full backbone (X_mid, Y_mid, Z_mid on a
    (N_cuerda x N_radios) grid), the inlet/outlet angle vectors, the
    solidity-derived blade count, and the summary hydraulic quantities
    used elsewhere in the app. On any physical infeasibility, returns
    ...
    # Integrate one streamline (fixed radius r) at a time, from the
    # leading edge (z=0, theta=0) to the trailing edge (z=-L_z).
    for i in range(N_radios):
        ...
        # Chordwise beta(z) distribution, blended from beta1 to beta2
        # using the selected interpolation law.
        t_dim = np.abs(z_vec) / L_z
        polinomio = evaluate_interpolation(interp_type, t_dim)
        beta_z = beta1 + (beta2 - beta1) * polinomio

        # Station-by-station integration of the circumferential twist:
        # d(theta)/dz = -cot(beta) / r, accumulated with a simple forward
        # difference between consecutive z stations.
        theta_rel = 0.0
    ...
    # Reference chord length, measured at the mean hydraulic radius, used
    # both to report a representative chord and to derive the blade count.
    ...
    # Blade count from a target solidity (chord-to-pitch ratio) at the
    # mean radius: pitch = (2*pi*R_m) / Z, so Z = (2*pi*R_m) / (chord/sigma).
...
def compute_deriaz_backbone(RPM, Q0, Hn, g, eta_h, eta_v, eta_o, sigma_target,
    ...
    """Diagonal (Deriaz) hydrodynamic model: the same Euler-equation-based
    approach as compute_kaplan_backbone() above, adapted to the diagonal
    (spherical) flow geometry that gives the Deriaz turbine its double-
    ...
    After the backbone is built, the reference chord and blade count are
    derived the same way as Kaplan, but evaluated at the log-mean
    spherical radius Re_mean = (Re_ext - Re_int) / ln(Re_ext / Re_int),
    which is the natural mean radius for a domain where the relevant
    circumference scales with cos(gamma)*Re rather than a constant r.
    ...
    # Integrate one streamline (fixed spherical radius Re) at a time, from
    # the leading edge (gamma=gamma1, theta=0) to the trailing edge
    # (gamma=gamma2).
    for i in range(N_radios):
        Re = Re_vec[i]
        ...
        # Chordwise (gamma-wise) beta distribution, blended from beta1 to
        # beta2 using the selected interpolation law.
        t = (gamma_vec - gamma1) / (gamma2 - gamma1)
        polinomio = evaluate_interpolation(interp_type, t)
        beta_gamma = beta1 + (beta2 - beta1) * polinomio

        # Station-by-station integration of the circumferential twist:
        # d(theta)/d(gamma) = -cot(beta) / cos(gamma), the spherical
        # analogue of the Kaplan cylindrical integration.
        theta_rel = 0.0
        for j in range(N_cuerda):
    ...
    # Reference chord length and blade count, evaluated at the log-mean
    # spherical radius (see the function docstring for why this specific
    # mean is the natural one here).
...
def compute_surface_normals(X, Y, Z):
    ...
    NOT USED ELSEWHERE IN THIS FILE - kept for reference only. This was
    the ORIGINAL method used to offset the blade thickness away from the
    mean surface, and it has a known geometric flaw: it mixes the
    chordwise curvature of the profile with the blade's spanwise twist
    rate between radial stations, which makes a solid blade generated
    this way visibly "lean sideways" as its thickness increases (the
    twist component grows proportionally with thickness). It was
    superseded by compute_thickness_direction() below, which instead
    builds a direction confined to the local chord/circumferential plane
    and does not exhibit this artifact."""
...
def compute_thickness_direction(X, Y, Z):
    ...
    Tu = np.stack([dXdu, dYdu, dZdu], axis=-1)   # tangent along the chord
    Tv = np.stack([dXdv, dYdv, dZdv], axis=-1)   # tangent along the radius/span
...
def build_solid_blade(X_mid, Y_mid, Z_mid, profile_type, t_rel_input,
    ...
    For each radial station i:
      1. The local chord length c_i is measured as the cumulative arc
         length of that streamline (s_backbone).
      2. The absolute reference thickness (t_abs_ref), fixed once from the
         ...
         - so the absolute (metric) thickness stays roughly constant
         across the span even though the relative thickness the profile
         generator actually uses varies with local chord.
...
QWidget#HeaderBanner {
    background-color: #F0F4F8; /* Light background (soft blue/gray) */
    border-bottom: 1px solid #D1D9E6; /* Optional subtle border to mark the boundary */
}

QLabel#HeaderTitle {
    color: #1A2536; /* Dark blue/gray for primary text */
    font-size: 15pt;
    font-weight: 600;
}
...
QGroupBox::title {
    ...
    left: 10px;
    padding: 0 6px;
    color: #8BBFEE; /* Muted blue, less harsh */
...
class KaplanDeriaz3DApp(QMainWindow):
    """Main application window: a three-tab parameter panel on the left
    (General / Hydrofoil-Mesh / Export) driving a dual PyVista 3D viewer
    and a results table on the right. See compute_turbine() for the
    top-level orchestration that ties the math core above to the GUI, and
    _render() for how the computed backbone is turned into a 3D display."""
    ...
    def apply_theme(self, dark):
        ...
        # Refresh the outer render container
        ...
        if self.is_computed and self.last_backbone is not None:
            ...
            # Force a visual refresh even if the scene is still empty
    ...
    def compute_turbine(self):
        ...
        every input widget, calls the appropriate hydrodynamic backbone
        function (compute_kaplan_backbone or compute_deriaz_backbone),
        thickens it into a solid via build_solid_blade(), stores the
        results on self for later export/rendering, fills the results
        table, and triggers the 3D render. Any physical infeasibility
        detected by the backbone functions is surfaced as a QMessageBox
        rather than silently producing invalid geometry."""
